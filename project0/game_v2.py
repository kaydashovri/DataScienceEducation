...
def random_predict(number:int=1) -> int:
    ...
    while True: 
        count +=1
        pridict_number = np.random.randint(1,101) # предполагаемое число 
        if number == pridict_number:
            break # выход из цикла, если угадали 
    return(count)

def score_game(randox_predict) -> int: 
    """За какое количество попыток в среднем из 1000 подходов угадывает наш алгоритм
    
    Args:
        randox_predict (_type_): функция угадывания 
    ...
    score = int(np.mean(count_ls)) # находим среднее количесво попыток  
